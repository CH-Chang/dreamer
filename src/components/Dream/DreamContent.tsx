...
export function DreamContent({ dream }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (category !== (dream.category || '')) data.category = category
    ...
    return (
      <div>
        ...
        <input
          ...
          placeholder="分類（選填）"
          className="w-full text-xs tracking-wider text-gray-400 bg-transparent border-b border-gray-200 pb-1 mb-4 focus:outline-none focus:border-gray-400 transition-colors placeholder-gray-200"
        />
        <textarea
          value={description}
